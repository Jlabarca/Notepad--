...
+(function setup() {
  ...
  function loadStyle() {
    const css = require('./stringified/style');
...
test('error annotations provided by inlined worker', (t) => {
  function getError(lang) {
    const editor = document.getElementById(`${lang}-editor`);
    const errors = editor.getElementsByClassName('ace_error');
  ...
  const max = 10; // give it a max of 10 seconds

  +(function javascript() {
    const err = getError('javascript');
