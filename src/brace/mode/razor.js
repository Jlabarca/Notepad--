...
ace.define('ace/mode/csharp_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/doc_comment_highlight_rules', 'ace/mode/text_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const DocCommentHighlightRules = acequire('./doc_comment_highlight_rules').DocCommentHighlightRules;
  const TextHighlightRules = acequire('./text_highlight_rules').TextHighlightRules;
...
ace.define('ace/mode/razor_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/doc_comment_highlight_rules', 'ace/mode/html_highlight_rules', 'ace/mode/csharp_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const lang = acequire('../lib/lang');
  const DocCommentHighlightRules = acequire('./doc_comment_highlight_rules').DocCommentHighlightRules;
  const HtmlHighlightRules = acequire('./html_highlight_rules').HtmlHighlightRules;
  const CSharpHighlightRules = acequire('./csharp_highlight_rules').CSharpHighlightRules;

  const blockPrefix = 'razor-block-';
  const RazorLangHighlightRules = function () {
    CSharpHighlightRules.call(this);

    const processPotentialCallback = function (value, stackItem) {
      if (typeof stackItem === 'function') { return stackItem(value); }

      return stackItem;
    };

    const inBraces = 'in-braces';
    this.$rules.start.unshift({
      regex: '[\\[({]',
      onMatch(value, state, stack) {
        const prefix = /razor-[^\-]+-/.exec(state)[0];
  ...
  const RazorHighlightRules = function () {
    ...
    for (const key in this.$rules) { this.$rules[key].unshift.apply(this.$rules[key], razorStartRules); }
...
ace.define('ace/mode/razor_completions', ['require', 'exports', 'module', 'ace/token_iterator'], (acequire, exports, module) => {
  const TokenIterator = acequire('../token_iterator').TokenIterator;
  ...
  const RazorCompletions = function () {
  ...
  (function () {
    ...
    this.getKeywordCompletions = function (state, session, pos, prefix) {
      return shortHands.concat(keywords).map(element => ({
        value: element,
        meta: 'keyword',
        score: Number.MAX_VALUE,
      }));
...
ace.define('ace/mode/razor', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/html', 'ace/mode/razor_highlight_rules', 'ace/mode/razor_completions', 'ace/mode/html_completions'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const HtmlMode = acequire('./html').Mode;
  const RazorHighlightRules = acequire('./razor_highlight_rules').RazorHighlightRules;
  const RazorCompletions = acequire('./razor_completions').RazorCompletions;
  const HtmlCompletions = acequire('./html_completions').HtmlCompletions;
  ...
  (function () {
    this.getCompletions = function (state, session, pos, prefix) {
      const razorToken = this.$completer.getCompletions(state, session, pos, prefix);
      const htmlToken = this.$htmlCompleter.getCompletions(state, session, pos, prefix);
