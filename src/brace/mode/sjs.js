...
ace.define('ace/mode/sjs_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/javascript_highlight_rules', 'ace/mode/text_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const JavaScriptHighlightRules = acequire('./javascript_highlight_rules').JavaScriptHighlightRules;
  const TextHighlightRules = acequire('./text_highlight_rules').TextHighlightRules;

  const SJSHighlightRules = function () {
    const parent = new JavaScriptHighlightRules({ noES6: true });
    const escapedRe = '\\\\(?:x[0-9a-fA-F]{2}|' + // hex
        'u[0-9a-fA-F]{4}|' + // unicode
        '[0-2][0-7]{0,2}|' + // oct
        '3[0-6][0-7]?|' + // oct
    ...
    const contextAware = function (f) {
      f.isContextAware = true;
      return f;
    };

    const ctxBegin = function (opts) {
      return {
        token: opts.token,
        regex: opts.regex,
        next: contextAware((currentState, stack) => {
    ...
    const embeddableRules = [];
    for (let i = 0; i < this.$rules.no_regex.length; i++) {
      const rule = this.$rules.no_regex[i];
...
ace.define('ace/mode/sjs', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/javascript', 'ace/mode/sjs_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/mode/behaviour/cstyle', 'ace/mode/folding/cstyle'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const JSMode = acequire('./javascript').Mode;
  const SJSHighlightRules = acequire('./sjs_highlight_rules').SJSHighlightRules;
  const MatchingBraceOutdent = acequire('./matching_brace_outdent').MatchingBraceOutdent;
  const CstyleBehaviour = acequire('./behaviour/cstyle').CstyleBehaviour;
  const CStyleFoldMode = acequire('./folding/cstyle').FoldMode;
