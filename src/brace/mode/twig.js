...
ace.define('ace/mode/twig_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/html_highlight_rules', 'ace/mode/text_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const lang = acequire('../lib/lang');
  const HtmlHighlightRules = acequire('./html_highlight_rules').HtmlHighlightRules;
  const TextHighlightRules = acequire('./text_highlight_rules').TextHighlightRules;

  const TwigHighlightRules = function () {
    HtmlHighlightRules.call(this);

    let tags = 'autoescape|block|do|embed|extends|filter|flush|for|from|if|import|include|macro|sandbox|set|spaceless|use|verbatim';
    tags = `${tags}|end${tags.replace(/\|/g, '|end')}`;
    const filters = 'abs|batch|capitalize|convert_encoding|date|date_modify|default|e|escape|first|format|join|json_encode|keys|last|length|lower|merge|nl2br|number_format|raw|replace|reverse|slice|sort|split|striptags|title|trim|upper|url_encode';
    const functions = 'attribute|constant|cycle|date|dump|parent|random|range|template_from_string';
    const tests = 'constant|divisibleby|sameas|defined|empty|even|iterable|odd';
    const constants = 'null|none|true|false';
    const operators = 'b-and|b-xor|b-or|in|is|and|or|not';
...
ace.define('ace/mode/twig', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/html', 'ace/mode/twig_highlight_rules', 'ace/mode/matching_brace_outdent'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const HtmlMode = acequire('./html').Mode;
  const TwigHighlightRules = acequire('./twig_highlight_rules').TwigHighlightRules;
  const MatchingBraceOutdent = acequire('./matching_brace_outdent').MatchingBraceOutdent;
