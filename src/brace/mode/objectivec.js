...
ace.define('ace/mode/objectivec_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/doc_comment_highlight_rules', 'ace/mode/c_cpp_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const DocCommentHighlightRules = acequire('./doc_comment_highlight_rules').DocCommentHighlightRules;
  const C_Highlight_File = acequire('./c_cpp_highlight_rules');
  const CHighlightRules = C_Highlight_File.c_cppHighlightRules;

  const ObjectiveCHighlightRules = function () {
    const escapedConstRe = "\\\\(?:[abefnrtv'\"?\\\\]|" +
                         '[0-3]\\d{1,2}|' +
                         '[4-7]\\d?|' +
                         '222|' +
                         'x[a-zA-Z0-9]+)';
    ...
    const cObj = new CHighlightRules();
    const cRules = cObj.getRules();
...
ace.define('ace/mode/objectivec', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/objectivec_highlight_rules', 'ace/mode/folding/cstyle'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const TextMode = acequire('./text').Mode;
  const ObjectiveCHighlightRules = acequire('./objectivec_highlight_rules').ObjectiveCHighlightRules;
