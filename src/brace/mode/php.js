...
ace.define('ace/mode/css_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/text_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  ...
  const pseudoClasses = exports.pseudoClasses = '(:)\\b(active|checked|disabled|empty|enabled|first-child|first-of-type|focus|hover|indeterminate|invalid|last-child|last-of-type|link|not|nth-child|nth-last-child|nth-last-of-type|nth-of-type|only-child|only-of-type|acequired|root|target|valid|visited)\\b';
...
ace.define('ace/mode/javascript_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/doc_comment_highlight_rules', 'ace/mode/text_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
...
ace.define('ace/mode/xml_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const TextHighlightRules = acequire('./text_highlight_rules').TextHighlightRules;
...
ace.define('ace/mode/html_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/css_highlight_rules', 'ace/mode/javascript_highlight_rules', 'ace/mode/xml_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const lang = acequire('../lib/lang');
  const CssHighlightRules = acequire('./css_highlight_rules').CssHighlightRules;
  const JavaScriptHighlightRules = acequire('./javascript_highlight_rules').JavaScriptHighlightRules;
  const XmlHighlightRules = acequire('./xml_highlight_rules').XmlHighlightRules;
...
ace.define('ace/mode/php_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/doc_comment_highlight_rules', 'ace/mode/text_highlight_rules', 'ace/mode/html_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const lang = acequire('../lib/lang');
  const DocCommentHighlightRules = acequire('./doc_comment_highlight_rules').DocCommentHighlightRules;
  const TextHighlightRules = acequire('./text_highlight_rules').TextHighlightRules;
  const HtmlHighlightRules = acequire('./html_highlight_rules').HtmlHighlightRules;

  const PhpLangHighlightRules = function () {
    const docComment = DocCommentHighlightRules;
    const builtinFunctions = lang.arrayToMap(('abs|acos|acosh|addcslashes|addslashes|aggregate|aggregate_info|aggregate_methods|aggregate_methods_by_list|aggregate_methods_by_regexp|' +
    ...
    const keywords = lang.arrayToMap(('abstract|and|array|as|break|case|catch|class|clone|const|continue|declare|default|do|else|elseif|enddeclare|endfor|endforeach|endif|' +
    ...
    const languageConstructs = lang.arrayToMap(('die|echo|empty|exit|eval|include|include_once|isset|list|acequire|acequire_once|return|print|unset').split('|'));

    const builtinConstants = lang.arrayToMap(('true|TRUE|false|FALSE|null|NULL|__CLASS__|__DIR__|__FILE__|__LINE__|__METHOD__|__FUNCTION__|__NAMESPACE__').split('|'));

    const builtinVariables = lang.arrayToMap(('$GLOBALS|$_SERVER|$_GET|$_POST|$_FILES|$_REQUEST|$_SESSION|$_ENV|$_COOKIE|$php_errormsg|$HTTP_RAW_POST_DATA|' +
        '$http_response_header|$argc|$argv').split('|'));
    const builtinFunctionsDeprecated = lang.arrayToMap(('key_exists|cairo_matrix_create_scale|cairo_matrix_create_translate|call_user_method|call_user_method_array|com_addref|com_get|' +
    ...
    const keywordsDeprecated = lang.arrayToMap(('cfunction|old_function').split('|'));

    const futureReserved = lang.arrayToMap([]);
    ...
    this.$rules = {
      start: [
        ...
        docComment.getStartRule('doc-start'),
        {
          ...
          token(value) {
            if (keywords.hasOwnProperty(value)) { return 'keyword'; } else if (builtinConstants.hasOwnProperty(value)) { return 'constant.language'; } else if (builtinVariables.hasOwnProperty(value)) { return 'variable.language'; } else if (futureReserved.hasOwnProperty(value)) { return 'invalid.illegal'; } else if (builtinFunctions.hasOwnProperty(value)) { return 'support.function'; } else if (value == 'debugger') { return 'invalid.deprecated'; } else
            if (value.match(/^(\$[a-zA-Z_\x7f-\uffff][a-zA-Z0-9_\x7f-\uffff]*|self|parent)$/)) { return 'variable'; }
            return 'identifier';
          },
          regex: /[a-zA-Z_$\x7f-\uffff][a-zA-Z0-9_\x7f-\uffff]*/,
        }, {
          onMatch(value, currentSate, state) {
      ...
      heredoc: [
        {
          onMatch(value, currentSate, stack) {
  ...
  const PhpHighlightRules = function () {
...
ace.define('ace/mode/php_completions', ['require', 'exports', 'module'], (acequire, exports, module) => {
  const functionMap = {
    abs: [
      'int abs(int number)',
  ...
  (function () {
    ...
    this.getArrayKeyCompletions = function (state, session, pos, prefix) {
      const line = session.getLine(pos.row).substr(0, pos.column);
      const variable = line.match(/(\$[\w]*)\[["']([^'"]*)$/i)[1];
...
ace.define('ace/mode/javascript', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/javascript_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/worker/worker_client', 'ace/mode/behaviour/cstyle', 'ace/mode/folding/cstyle'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  ...
  (function () {
    ...
    this.createWorker = function (session) {
      const worker = new WorkerClient(['ace'], require('../worker/javascript'), 'JavaScriptWorker');
...
ace.define('ace/mode/css_completions', ['require', 'exports', 'module'], (acequire, exports, module) => {
  const propertyMap = {
    background: { '#$0': 1 },
...
ace.define('ace/mode/behaviour/css', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/behaviour', 'ace/mode/behaviour/cstyle', 'ace/token_iterator'], (acequire, exports, module) => {
  const oop = acequire('../../lib/oop');
  const Behaviour = acequire('../behaviour').Behaviour;
  const CstyleBehaviour = acequire('./cstyle').CstyleBehaviour;
...
ace.define('ace/mode/css', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/css_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/worker/worker_client', 'ace/mode/css_completions', 'ace/mode/behaviour/css', 'ace/mode/folding/cstyle'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const TextMode = acequire('./text').Mode;
  const CssHighlightRules = acequire('./css_highlight_rules').CssHighlightRules;
  const MatchingBraceOutdent = acequire('./matching_brace_outdent').MatchingBraceOutdent;
  const WorkerClient = acequire('../worker/worker_client').WorkerClient;
  const CssCompletions = acequire('./css_completions').CssCompletions;
  const CssBehaviour = acequire('./behaviour/css').CssBehaviour;
  const CStyleFoldMode = acequire('./folding/cstyle').FoldMode;
  ...
  (function () {
    ...
    this.createWorker = function (session) {
      const worker = new WorkerClient(['ace'], require('../worker/css'), 'Worker');
      worker.attachToDocument(session.getDocument());
...
ace.define('ace/mode/behaviour/xml', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/behaviour', 'ace/token_iterator', 'ace/lib/lang'], (acequire, exports, module) => {
  const oop = acequire('../../lib/oop');
  const Behaviour = acequire('../behaviour').Behaviour;
  const TokenIterator = acequire('../../token_iterator').TokenIterator;
  const lang = acequire('../../lib/lang');
...
ace.define('ace/mode/folding/mixed', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/folding/fold_mode'], (acequire, exports, module) => {
  const oop = acequire('../../lib/oop');
  const BaseFoldMode = acequire('./fold_mode').FoldMode;
...
ace.define('ace/mode/folding/xml', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/range', 'ace/mode/folding/fold_mode', 'ace/token_iterator'], (acequire, exports, module) => {
  const oop = acequire('../../lib/oop');
  const lang = acequire('../../lib/lang');
  const Range = acequire('../../range').Range;
  const BaseFoldMode = acequire('./fold_mode').FoldMode;
...
ace.define('ace/mode/folding/html', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/folding/mixed', 'ace/mode/folding/xml', 'ace/mode/folding/cstyle'], (acequire, exports, module) => {
  const oop = acequire('../../lib/oop');
  const MixedFoldMode = acequire('./mixed').FoldMode;
  const XmlFoldMode = acequire('./xml').FoldMode;
  const CStyleFoldMode = acequire('./cstyle').FoldMode;
...
ace.define('ace/mode/html_completions', ['require', 'exports', 'module', 'ace/token_iterator'], (acequire, exports, module) => {
  const TokenIterator = acequire('../token_iterator').TokenIterator;
...
ace.define('ace/mode/html', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/lib/lang', 'ace/mode/text', 'ace/mode/javascript', 'ace/mode/css', 'ace/mode/html_highlight_rules', 'ace/mode/behaviour/xml', 'ace/mode/folding/html', 'ace/mode/html_completions', 'ace/worker/worker_client'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const lang = acequire('../lib/lang');
  const TextMode = acequire('./text').Mode;
  const JavaScriptMode = acequire('./javascript').Mode;
  const CssMode = acequire('./css').Mode;
  const HtmlHighlightRules = acequire('./html_highlight_rules').HtmlHighlightRules;
  const XmlBehaviour = acequire('./behaviour/xml').XmlBehaviour;
  const HtmlFoldMode = acequire('./folding/html').FoldMode;
  const HtmlCompletions = acequire('./html_completions').HtmlCompletions;
  const WorkerClient = acequire('../worker/worker_client').WorkerClient;
  ...
  (function () {
    ...
    this.createWorker = function (session) {
      ...
      const worker = new WorkerClient(['ace'], require('../worker/html'), 'Worker');
      worker.attachToDocument(session.getDocument());
...
ace.define('ace/mode/php', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/php_highlight_rules', 'ace/mode/php_highlight_rules', 'ace/mode/matching_brace_outdent', 'ace/range', 'ace/worker/worker_client', 'ace/mode/php_completions', 'ace/mode/behaviour/cstyle', 'ace/mode/folding/cstyle', 'ace/unicode', 'ace/mode/html', 'ace/mode/javascript', 'ace/mode/css'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const TextMode = acequire('./text').Mode;
  const PhpHighlightRules = acequire('./php_highlight_rules').PhpHighlightRules;
  const PhpLangHighlightRules = acequire('./php_highlight_rules').PhpLangHighlightRules;
  const MatchingBraceOutdent = acequire('./matching_brace_outdent').MatchingBraceOutdent;
  const Range = acequire('../range').Range;
  const WorkerClient = acequire('../worker/worker_client').WorkerClient;
  const PhpCompletions = acequire('./php_completions').PhpCompletions;
  const CstyleBehaviour = acequire('./behaviour/cstyle').CstyleBehaviour;
  const CStyleFoldMode = acequire('./folding/cstyle').FoldMode;
  const unicode = acequire('../unicode');
  const HtmlMode = acequire('./html').Mode;
  const JavaScriptMode = acequire('./javascript').Mode;
  const CssMode = acequire('./css').Mode;

  const PhpMode = function (opts) {
  ...
  const Mode = function (opts) {
    if (opts && opts.inline) {
      const mode = new PhpMode();
  ...
  (function () {
    this.createWorker = function (session) {
      const worker = new WorkerClient(['ace'], 'ace/mode/php_worker', 'PhpWorker');
      worker.attachToDocument(session.getDocument());
