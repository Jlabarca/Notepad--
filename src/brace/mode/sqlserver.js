...
ace.define('ace/mode/sqlserver_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/doc_comment_highlight_rules', 'ace/mode/text_highlight_rules'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  ...
  const SqlServerHighlightRules = function () {
    ...
    keywords = keywords.filter((value, index, self) => logicalOperators.split('|').indexOf(value) === -1 && builtinFunctions.split('|').indexOf(value) === -1 && dataTypes.split('|').indexOf(value) === -1);
...
ace.define('ace/mode/folding/sqlserver', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/range', 'ace/mode/folding/cstyle'], (acequire, exports, module) => {
  const oop = acequire('../../lib/oop');
  const Range = acequire('../../range').Range;
  const BaseFoldMode = acequire('./cstyle').FoldMode;
  ...
  (function () {
    ...
    this.getBeginEndBlock = function (session, row, column, matchSequence) {
      ...
      let depth = 1;
      const re = /(\bCASE\b|\bBEGIN\b)|(\bEND\b)/i;
      while (++row < maxRow) {
        line = session.getLine(row);
        const m = re.exec(line);
        if (!m) continue;
...
ace.define('ace/mode/sqlserver', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/sqlserver_highlight_rules', 'ace/mode/folding/sqlserver'], (acequire, exports, module) => {
  const oop = acequire('../lib/oop');
  const TextMode = acequire('./text').Mode;
  const SqlServerHighlightRules = acequire('./sqlserver_highlight_rules').SqlHighlightRules;
  const SqlServerFoldMode = acequire('./folding/sqlserver').FoldMode;
